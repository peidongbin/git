...
namespace Monitorsever
{
    class VlcPlayer
    {
        ...
        public VlcPlayer(string pluginPath)
        {
            ...
            //string[] arguments = { "-I", "dummy", "--sout=#duplicate{dst=std{accs=file,mux=ts,dst=D:\\1.h264}}", "--no-video-title", plugin_arg };//添加录制的命令
            
            libvlc_instance_ = LibVlcAPI.libvlc_new(arguments);

            libvlc_media_player_ = LibVlcAPI.libvlc_media_player_new(libvlc_instance_);
        }
        public VlcPlayer(string pluginPath, string filename)
        {
            string plugin_arg = "--plugin-path=" + pluginPath;
            //string[] arguments = { "-I", "dummy", "--ignore-config", "--no-video-title", plugin_arg };
            string[] arguments = { "-I", "dummy", "--sout=#duplicate{dst=std{accs=file,mux=ts,dst=" + filename + "}}", "--no-video-title", plugin_arg };//添加录制的命令
            //string[] arguments = { "-I", "dummy", "--sout=#duplicate{dst=std{accs=file,mux=ts,dst=" + filename + "}}", "--no-video-title", "-vvv", "--file-logging", plugin_arg };//添加录制的命令
        ...
        {
            //IntPtr libvlc_media = LibVlcAPI.libvlc_media_new_location(libvlc_instance_, filePath);//播放url
            IntPtr libvlc_media = LibVlcAPI.libvlc_media_new_path(libvlc_instance_, filePath);//播放本地文件
        ...
        //    {
        //        //LibVlcAPI.libvlc_media_add_option("--sout=#duplicate{dst=display,dst=std{accs=file,mux=ts,dst= D://1.mp4}}");
        //        LibVlcAPI.libvlc_media_add_option(libvlc_media_player_, " --ignore-config");
        ...
        //     return LibVlcAPI.GetLastError();
         
        //}
        public int getid()
        {

            return LibVlcAPI.Getid(libvlc_media_player_);
    ...
    internal static class LibVlcAPI
    {
        ...
        //[DllImport("libvlc", EntryPoint = "libvlc_errmsg",CallingConvention = CallingConvention.Cdecl)]
        ...
        public static extern int Getid(IntPtr libvlc_media_player);
        // 当前播放的时间
        [DllImport("libvlc", CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
        [SuppressUnmanagedCodeSecurity]
